...
def model_train(x_train,y_train,x_test, y_test, model):
	checkpoint = ModelCheckpoint("ff-{epoch:02d}-{val_loss:.2f}.hdf5",  mode='auto', period=2000, monitor='val_acc')
...
def model_test(x_test, y_test, x_train, y_train):
	json_file = open('ffModel22nd.json', 'r')
	loaded_model_json = json_file.read()
	json_file.close()
	loaded_model = model_from_json(loaded_model_json)
	# load weights into new model
	loaded_model.load_weights("ff-100-4.86.hdf5")
...
def savedata(model): 
	...
	with open("ffModel22nd.json", "w") as json_file:
    		json_file.write(model_json)
    	model.save_weights("ffModel22nd.h5")
...
def preload():
	...
	mod = model()
	rootdir = '/home/fractaluser/Desktop/SpeakerAI/data/test/'
	traindata = training_data(rootdir)
	x_train = traindata[0]
	y_train = traindata[1]
	rootdir1 = '/home/fractaluser/Desktop/SpeakerAI/data/test/'
	...
	pickle.dump(svm_x_train,open('x_train123.p','wb'))
	pickle.dump(svm_y_train,open('y_train123.p','wb'))
	pickle.dump(svm_x_test,open('x_test123.p','wb'))
	pickle.dump(svm_y_test,open('y_test123.p','wb'))

def dataPreload():
	global svm_x_train,svm_y_train,svm_x_test,svm_y_test
	svm_x_train = pickle.load(open('x_train.p','rb'))
	svm_y_train = pickle.load(open('y_train.p','rb'))
	svm_x_test = pickle.load(open('x_test.p','rb'))
	svm_y_test = pickle.load(open('y_test.p','rb'))
